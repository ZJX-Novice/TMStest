...
def convert_and_send_init_data(file_name):  # 处理数据
    ...
    for line in lines:
        ...
        first_frame_data = [
            ...
            first_frame_SID,
            frame_ID,
            ' '.join([line_bytes[0:1].hex().upper(), line_bytes[1:2].hex().upper(), line_bytes[2:3].hex().upper()])
            # D1, D2, D3
        ]
...
if __name__ == "__main__":
    ...
    if (bool(ret)):
        ...
        ui.textEdit_3.insertPlainText('\n')
        ui.textEdit_3.insertPlainText("--Firmware Version: v%d.%d.%d" % (
        (USB2XXXInfo.FirmwareVersion >> 24) & 0xFF, (USB2XXXInfo.FirmwareVersion >> 16) & 0xFF,
        USB2XXXInfo.FirmwareVersion & 0xFFFF))
        ui.textEdit_3.insertPlainText('\n')
        ui.textEdit_3.insertPlainText("--Hardware Version: v%d.%d.%d" % (
